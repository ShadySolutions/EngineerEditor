			...
			while (curItem < completionData.Length && yPos < Height) {
				...
				if (drawingBackground.IntersectsWith(pe.ClipRectangle)) {
					...
					if (curItem == selectedItem) {
                        g.FillRectangle(ThemeHardcode.BackBrush, drawingBackground);
					} else {
						g.FillRectangle(ThemeHardcode.SelectedBush , drawingBackground);
					}
					...
					// draw text
					if (curItem == selectedItem) {
						g.DrawString(completionData[curItem].Text, Font, ThemeHardcode.SelectedText, xPos, yPos);
					} else {
						g.DrawString(completionData[curItem].Text, Font, ThemeHardcode.DefaultText, xPos, yPos);
					}
				}
